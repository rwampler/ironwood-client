...
export default class Noise {
  ...
  static white (width:number, height:number, seed:string | null): Float32Array {
    const rng = random.clone(seedrandom(seed ?? "default-seed"));
    var noise = new Float32Array(width * height);
  ...
  static smooth (width:number, height:number, octave:number, options:SmoothNoiseOptions | null): Float32Array {
    const whiteNoise = options?.whiteNoise ?? Noise.white(width, height, options?.seed ?? "default-seed");
    const noise = new Float32Array(width * height);
    const samplePeriod = Math.pow(2, octave ?? 0);
  ...
  static perlin (width:number, height:number, options: Options | null): Float32Array  {
    ...
    const perlinNoise = new Float32Array(width * height);
